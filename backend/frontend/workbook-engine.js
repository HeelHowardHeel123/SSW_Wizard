...
// ranges, guarding cross-sheet refs (those preceded by "!").
function bumpFormulaRefs(xml, at, N) {
  return xml.replace(/<f\b[^>]*>[\s\S]*?<\/f>|<f\b[^>]*\/>/g, (seg) =>
    seg.replace(/(?<![!A-Za-z0-9])(\$?[A-Z]{1,3}\$?)(\d+)/g, (m, c, d) => {
      const n = +d;
      return n >= at ? c + (n + N) : m;
    })
  );
...
function buildCell(addr, attrs, value, type) {
...
function patchCellInSheet(xml, addr, value, type) {
  const re = new RegExp(`<c r="${addr}"([^>]*?)(?:/>|>[\\s\\S]*?</c>)`);
  const m = xml.match(re);
  if (m) {
    const cell = buildCell(addr, m[1], value, type);
  ...
  const cell = buildCell(addr, "", value, type);
  if (cell == null) return xml;
...
export async function generateWorkbook(templateBuf, sheetName, values, deleteSheets = [], opts = {}) {
  ...
  for (const v of values) {
    ...
    sheetXml = patchCellInSheet(sheetXml, v.addr, v.value, v.type);
  ...
  if (opts.cellPatches && opts.cellPatches.length) {
    ...
    for (const [sn, patches] of Object.entries(bySheet)) {
      ...
      for (const p of patches) {
        ...
        px = patchCellInSheet(px, p.addr, p.value, p.type);
        written++;
      }
